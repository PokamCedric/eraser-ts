"""
Algorithm v6 - AVEC OPTIMISATION DE COMPLEXITÉ
Basé sur algo5 + optimisation du choix de référence (parent vs enfant)
...
4. Optimisation: choisir la référence (parent ou enfant) selon la complexité minimale
"""
...
for iteration_num, entity_name in enumerate(entity_order, 1):
    ...
    if entity_already_placed and unplaced_parents:
        ...
        if entity_current_layer < required_layer:
            print(f"   -> Conflit détecté: '{entity_name}' au Layer {entity_current_layer}, devrait être au Layer {required_layer}")

            # === NOUVELLE LOGIQUE: OPTIMISATION DU CHOIX DE RÉFÉRENCE ===
            ...
            # CAS 1: Déplacer l'enfant (entity_name) vers la droite
            # On déplace entity_name + ses descendants + leurs clusters
            all_descendants_child = find_all_children(entity_name)
            entity_parents = set(cluster_left)

            entities_to_move_child = {entity_name}
            for descendant in all_descendants_child:
                if find_layer_index(descendant) is not None:
                    entities_to_move_child.add(descendant)
                    for parent in find_all_parents(descendant):
                        if find_layer_index(parent) is not None and parent not in entity_parents:
                            entities_to_move_child.add(parent)

            # Calculer la complexité (somme des connexions)
            complexity_child = sum(connection_count.get(e, 0) for e in entities_to_move_child)

            # CAS 2: Déplacer les parents vers la gauche
            # On déplace les parents placés + leurs ancêtres + leurs clusters
            placed_parents = [p for p in cluster_left if find_layer_index(p) is not None]

            entities_to_move_parent = set()
            for parent in placed_parents:
                entities_to_move_parent.add(parent)
                # Trouver tous les ancêtres (parents du parent)
                ancestors = find_all_parents(parent)
                for ancestor in ancestors:
                    if find_layer_index(ancestor) is not None:
                        entities_to_move_parent.add(ancestor)
                        # Ajouter les descendants de cet ancêtre (sauf entity_name et ses descendants)
                        for child in find_all_children(ancestor):
                            if find_layer_index(child) is not None and child != entity_name and child not in all_descendants_child:
                                entities_to_move_parent.add(child)

            complexity_parent = sum(connection_count.get(e, 0) for e in entities_to_move_parent)

            print(f"      OPTIMISATION:")
            print(f"        Cas 1 (déplacer enfant '{entity_name}'): {len(entities_to_move_child)} entités, complexité = {complexity_child}")
            print(f"        Cas 2 (déplacer parents {placed_parents}): {len(entities_to_move_parent)} entités, complexité = {complexity_parent}")

            # Choisir le cas avec la complexité minimale
            if complexity_child <= complexity_parent:
                print(f"      -> Choix: CAS 1 (déplacer enfant)")
                entities_to_move = list(entities_to_move_child)
                shift = required_layer - entity_current_layer

                print(f"      Entites a deplacer en cascade: {entities_to_move}")
                ...
                print(f"      -> Choix: CAS 2 (déplacer parents)")
                entities_to_move = list(entities_to_move_parent)

                # Calculer le décalage nécessaire: déplacer les parents vers la gauche
                # Le parent le plus à droite doit être placé avant entity_current_layer
                max_parent_current = max(find_layer_index(p) for p in placed_parents if find_layer_index(p) is not None)
                shift = entity_current_layer - 1 - max_parent_current

                print(f"      Entites a deplacer en cascade: {entities_to_move}")
                print(f"      Decalage: {shift} layers (vers la gauche)")
                ...
                for entity_to_move in entities_to_move:
                    ...
                    if current_pos is not None:
                        ...
                        if new_pos < 0:
                            # Insérer de nouveaux layers à gauche
                            for _ in range(abs(new_pos)):
                                layers.insert(0, [])
                            new_pos = 0
